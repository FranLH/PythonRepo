...
class Layer:
    def __init__(self, size:int, xPos:int, network:NeuralNetwork):
        ...
        if xPos%2==0:
            ...
            for i in range(size):
                self.contents.append(Connection(random.random(),i))
    def __repr__(self):
...
for x in range(800): # Simple example that compares if a number is larger than another one
    modifications = []
    bigCost = 0
    for _ in range(100): 
        num1 = random.randint(0,100)
        num2 = random.randint(0,100)
        LNN.SetInputs([num1/100, num2/100])
        LNN.Run(True)
        modifications.append(LNN.Backpropagation([int(num1>num2), int(num2>num1)]))
        bigCost+=LNN.GetCost([int(num1>num2), int(num2>num1)])
    ...
    if x > 750:
        ...
        print([int(num1>num2), int(num2>num1)], num1,num2, "-->", LNN.GetOutputs()[0], LNN.GetOutputs()[1])
        #print(modifications)
        print("cost:",LNN.GetCost([int(num1>num2), int(num2>num1)]))
