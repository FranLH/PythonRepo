...
#             #print(activationDerivatives[-1][0])
        ...
        # Calculates the gradient for the connections to the next layer
            for connY, connection in enumerate(connectionsLayer.contents):
                ...
                nextNeuronY = connY // neuronLayer.size
                changes[layer+1][connY] = activationDerivatives[layer+2][nextNeuronY] * nextNeuronLayer.contents[nextNeuronY].activation * changes[layer+2][nextNeuronY]
        ...
        for connY, connection in enumerate(self.layers[1].contents): # Handles the gradient for the weights connected to the input layer
            previousNeuronY = connY % neuronLayer.size
            nextNeuronY = connY // neuronLayer.size
            changes[1][connY] = activationDerivatives[2][nextNeuronY] * nextNeuronLayer.contents[nextNeuronY].activation * changes[2][nextNeuronY]
        return(changes)
...
for j in range(800):
    modifications = []
    bigCost = 0
    for i in range(100): # Simple Neural network test, trains a model with a single neuron to recognize if a number is smaller or bigger than 50
