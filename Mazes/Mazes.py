...
#                         pass
    ...
    def Generate(self, startPos, maxDepth, seed=0):
        ...
        while i < maxDepth:
            ...
            options = []
            if pos.x>2 and self.Get(dataClone, pos+vec2(-2,0)) == 1: # Move left
                options.append(vec2(-2,0))
            if pos.x<self.size[0]-3 and self.Get(dataClone, pos+vec2(2,0)) == 1: # Move right
                options.append(vec2(2,0))
            if pos.y>2 and self.Get(dataClone, pos+vec2(0,-2)) == 1: # Move left
                options.append(vec2(0,-2))
            if pos.y<self.size[1]-3 and self.Get(dataClone, pos+vec2(0,2)) == 1: # Move right
                options.append(vec2(0,2))
            ...
            if options != []:
                ...
                if pos.x>2 and self.Get(dataClone, pos+vec2(-2,0)) == 0 and self.Get(dataClone, pos+vec2(-1,0)) == 0: # Move left
                    options.append(vec2(-2,0))
                if pos.x<self.size[0]-3 and self.Get(dataClone, pos+vec2(2,0)) == 0 and self.Get(dataClone, pos+vec2(1,0)) == 0: # Move right
                    options.append(vec2(2,0))
                if pos.y>2 and self.Get(dataClone, pos+vec2(0,-2)) == 0 and self.Get(dataClone, pos+vec2(0,-1)) == 0: # Move left
                    options.append(vec2(0,-2))
                if pos.y<self.size[1]-3 and self.Get(dataClone, pos+vec2(0,2)) == 0 and self.Get(dataClone, pos+vec2(0,1)) == 0: # Move right
                    options.append(vec2(0,2))
                if options != []:
