...
class Maze:
    def __init__(self, pos:vec3, size:list):
...
#                         pygame.draw.rect(window.surface, (color,color,color), [cam.ToScreen(self.pos+vec2(x,y)).ToList(), (vec2(1,1)*math.ceil(cam.SizeZoom(1))).ToList()])
    ...
    def Get(self,data, pos:vec3):
        lx = len(data)
        ly = len(data[0])
        lz = len(data[0][0])
        if pos.x >= 0 and pos.x < lx and pos.y >=0 and pos.y < ly and pos.z >= 0 and pos.z < lz:
#             print(pos)
    ...
    def Fix(self, startPos):
        ...
        for x in range(self.size[0]):
            for y in range(self.size[1]):
                for z in range(self.size[2]):
                    if x>0 and x<(self.size[0]-1) and y>0 and y<(self.size[1]-1) and x%2==EvenX and y%2==EvenY and self.data[x][y][z] == 1:
    #                     try:
                        fixed[x][y-1][z]=1
                        fixed[x][y+1][z]=1
                        fixed[x-1][y][z]=1
...
#                         pass
    ...
    def Generate(self, startPos, maxDepth, maxAnts, antProb, seed=0):
        ...
        if maxDepth == 0: # Estimates the max amount of iterations needed to create the maze
            maxDepth=math.ceil(self.size[0]/2-1)*math.ceil(self.size[1]/2-1)*math.ceil(self.size[2])*2
        ...
        i=0
        newAnt = [False, vec3(0,0,0)]
        while i < maxDepth and len(ants)!=0:
            for antID in range(len(ants)):
                pos = ants[antID]
                length=depthClone[pos.x][pos.y][pos.z]
                options = []
