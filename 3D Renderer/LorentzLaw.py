...
class HollowObject:
    def __init__(self,x,y,z, lines, color, width):
        self.x, self.y, self.z, self.lines, self.color, self.width = x, y, z, lines, color, width
#   x
# 1 0 0    x = x
# 0 c -s   y = y*cX + z*-sX
# 0 s c    z = y*sX + z*cX
...
H1 = HollowObject(0,20,0,[[p2,p3],[p1,p3],[p2,p4],[p4,p1],[p5,p7],[p6,p7],[p6,p8],[p8,p5],[p1,p5],[p2,p6],[p3,p7],[p4,p8]], (255,255,255), 1)
YPos = [[point(0,0,0), point(0,20,0)],[point(0,20,0), point(0,18,-2)],[point(0,20,0), point(0,18,2)]]
YNeg = [[point(0,0,0), point(0,-20,0)],[point(0,-20,0), point(0,-18,-2)],[point(0,-20,0), point(0,-18,2)]]
XPos = [[point(0,0,0), point(20,0,0)],[point(20,0,0), point(18,2,0)],[point(20,0,0), point(18,-2,0)]]
XNeg = [[point(0,0,0), point(-20,0,0)],[point(-20,0,0), point(-18,2,0)],[point(-20,0,0), point(-18,-2,0)]]
ZPos = [[point(0,0,0), point(0,0,20)],[point(0,0,20), point(2,0,18)],[point(0,0,20), point(-2,0,18)]]
ZNeg = [[point(0,0,0), point(0,0,-20)],[point(0,0,-20), point(2,0,-18)],[point(0,0,-20), point(-2,0,-18)]]

H1 = HollowObject(100,0,0,XPos, (0,255,0), 4)
H2 = HollowObject(100,0,0,ZNeg, (255,0,0), 4)
H3 = HollowObject(100,0,0,YPos, (0,0,255), 4)

HollowObjects = [H1,H2, H3]
...
def Render():
    for obj in HollowObjects:
        ...
        for line in obj.lines:
            ...
            processedLine = []
            for p in line:
                P = point(*Localize(CamPos, CamRotation,point(obj.x+p.x, obj.y+p.y, obj.z+p.z)))
                y = (P.y*FL)/P.x
                x = (P.z*FL)/P.x 
                if P.x <= 0: # Prevents objects rendering from behind the camera
                    ...
                    processedLine.append([x,y]) # TAB
            if valid == 0:
                processed.append(processedLine)
        for line in processed:
            pygame.draw.line(window,obj.color,ToScreen(line[0],WindowSize),ToScreen(line[1],WindowSize), obj.width)
    processed = []
