...
#         pygame.draw.circle(window, (255,0,0), self.Objective.ToList(), 4) # Renders the objective
    ...
    def Forwards(self,objective:vec2):
        if self.pos.Dist(objective)>2: # I can add a distance check here, so it doesn't keep trying when it has arrived at the objective
            ...
            #self.Backwards() # To make the snakes we just remove the backwards call
    def Backwards(self):
...
S1 = Snake(vec2(420,300),40,[4]*39, 2, 1, 3, 10)
S = Snake(vec2(180,300),10,[45,45,45,37,37,37,30,30,22], 4, 0.5, 4, 8)
Snakes = [S1]
#L.Backwards()
#print(vec2(23,45)/2)
...
while running:
    ...
    for snake in Snakes:
        for i in range(1):
            snake.Forwards(vec2(MousePos[0],MousePos[1]))
        snake.Render()
    pygame.display.flip()
